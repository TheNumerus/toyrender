...
float compute_spatial_variance(ivec2 uv_int) {
    ...
    vec3 base_normal = normalize(texture(textures[3], uv).xyz * 2.0 - 1.0);
    float base_depth = get_linear_depth(textures[4], uv);
    float luma_base = luminance(texture(textures[0], uv).xyz);
    float gradient = get_depth_gradient(uv_int.x, uv_int.y, textures[4]);
    ...
    for (int x = -3; x <= 3; x++) {
        for (int y = -3; y <= 3; y++) {
            ...
            if (uv.x >= 0 || uv.y >= 0 || uv.x < size.x || uv.y < size.y) {
                ...
                vec4 current = texture(textures[0], texture_uv);
                float luma_sample = luminance(current.xyz);
                vec3 normal_sample = normalize(texture(textures[3], texture_uv).xyz * 2.0 - 1.0);
                float depth_sample = get_linear_depth(textures[4], texture_uv);
...
void main() {
    ...
    if (x <= size.x && y <= size.y) {
        ...
        if (globals.debug == DEBUG_DIRECT_VARIANCE || globals.debug == DEBUG_INDIRECT_VARIANCE) {
            imageStore(images[0], ivec2(x, y), vec4(vec3(variance * 5.0), 1.0));
            return;
        }
        ...
        vec4 prev = max(texture(textures[1], uv_reprojected), vec4(0.000001));
        imageStore(images[0], ivec2(x, y), mix(prev, vec4(current.xyz, variance), blend_factor));
    }
}